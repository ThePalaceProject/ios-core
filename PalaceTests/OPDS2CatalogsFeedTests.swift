...
class OPDS2CatalogsFeedTests: XCTestCase {
  ...
  let gplAuthUrl = Bundle(for: OPDS2CatalogsFeedTests.self)
    .url(forResource: "gpl_authentication_document", withExtension: "json")!
  let aclAuthUrl = Bundle(for: OPDS2CatalogsFeedTests.self)
    .url(forResource: "acl_authentication_document", withExtension: "json")!
  let dplAuthUrl = Bundle(for: OPDS2CatalogsFeedTests.self)
    .url(forResource: "dpl_authentication_document", withExtension: "json")!
  let nyplAuthUrl = Bundle(for: OPDS2CatalogsFeedTests.self)
  ...
  func disabledTestLoadAllAuthenticationDocuments() {
    ...
    do {
      ...
      for publication in feed.catalogs {
        do {
          let authDocumentUrl = publication.links
          ...
          _ = try OPDS2AuthenticationDocument.fromData(authData)
        } catch {
  ...
  func testInitAccountsWithPublication() {
    do {
      ...
      let gpl = Account(
        publication: feed.catalogs.first(where: { $0.metadata.title == "Glendora Public Library" })!,
        imageCache: MockImageCache()
      )
      let acl = Account(
        publication: feed.catalogs.first(where: { $0.metadata.title == "Alameda County Library" })!,
        imageCache: MockImageCache()
      )
      let dpl = Account(
        publication: feed.catalogs.first(where: { $0.metadata.title == "Digital Public Library of America" })!,
        imageCache: MockImageCache()
      )
      let nypl = Account(
      ...
      XCTAssertEqual(gpl.supportEmail!.rawValue, "library@glendoralibrary.org")
      ...
      XCTAssertEqual(acl.supportEmail!.rawValue, "simplye@aclibrary.org")
      ...
      XCTAssertEqual(dpl.supportEmail!.rawValue, "ebooks@dp.la")
      ...
      XCTAssertEqual(
        nypl.subtitle,
        "Inspiring lifelong learning, advancing knowledge, and strengthening our communities."
      )
      ...
      XCTAssertEqual(nypl.supportEmail!.rawValue, "simplyehelp@nypl.org")
      XCTAssertEqual(
        nypl.authenticationDocumentUrl,
  ...
  func testAccountSetAuthenticationDocument() {
    do {
      ...
      XCTAssertEqual(
        gpl.details?.signUpUrl,
        URL(string: "https://catalog.ci.glendora.ca.us/polaris/patronaccount/selfregister.aspx?ctx=3.1033.0.0.1")
      )
      ...
      XCTAssertEqual(
        nypl.details?.getLicenseURL(.contentLicenses),
