...
// MARK: - MyBooksDownloadCenterTests
...
class MyBooksDownloadCenterTests: XCTestCase {
  ...
  func testBorrowBook() {
    ...
    swizzle(selector: #selector(TPPOPDSFeed
        .swizzledURL_Success(_:shouldResetCache:useTokenIfAvailable:completionHandler:)
    ))
  ...
  func testBorrowBook_withReauthentication() {
    let expectation = expectation(description: "Books is sent to downloading state")
    ...
    swizzle(selector: #selector(TPPOPDSFeed
        .swizzledURL_Error(_:shouldResetCache:useTokenIfAvailable:completionHandler:)
    ))
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
      ...
      XCTAssertTrue(
        [.downloading, .downloadSuccessful].contains(bookState),
...
extension TPPOPDSFeed {
  @objc func swizzledURL_Success(
    _: URL,
    shouldResetCache _: Bool,
    useTokenIfAvailable _: Bool,
