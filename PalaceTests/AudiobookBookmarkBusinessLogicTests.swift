...
class AudiobookBookmarkBusinessLogicTests: XCTestCase {
  ...
  func testSaveListeningPosition() {
    ...
    sut.saveListeningPosition(at: localTestBookmark) { _ in
      expectation.fulfill()
  ...
  func testFetchBookmarksDuplicate_LocalAndRemote() {
    ...
    var localTestBookmarkTwo = TrackPosition(track: tracks.tracks[1], timestamp: 111_000, tracks: tracks)
    ...
    mockRegistry.preloadData(
      bookIdentifier: fakeBook.identifier,
      locations: registryTestBookmarks.compactMap { $0.toAudioBookmark().toTPPBookLocation() }
    )

    // Setup mock annotations
    let remoteBookmarks = remoteTestBookmarks.compactMap { TestBookmark(
      annotationId: $0.annotationId,
      value: $0.toAudioBookmark().toTPPBookLocation()!.locationString
    ) }
    ...
    sut.fetchBookmarks(for: tracks, toc: [Chapter(
      title: "",
      position: localTestBookmark,
      duration: 10.0
    )]) { bookmarks in
  ...
  func testFetchBookmarks_RemoteOnly() {
    ...
    var testBookmarkTwo = TrackPosition(track: tracks.tracks[1], timestamp: 111_000, tracks: tracks)
    ...
    sut.syncBookmarks(localBookmarks: registryTestBookmarks.compactMap { $0.toAudioBookmark() }) { _ in
      DispatchQueue.main.async {
        ...
        expectedLocalBookmarks.forEach { expectedBookmark in
          XCTAssertFalse(localBookmarks
            .filter { $0.locationString == expectedBookmark.toAudioBookmark().toTPPBookLocation()?.locationString }
            .isEmpty
          )
        }
      }
  ...
  func testBookmarkSync_LocalToRemote() {
    ...
    let localTestBookmark = TrackPosition(track: tracks.tracks[0], timestamp: 1000, tracks: tracks)
    let localTestBookmarkTwo = TrackPosition(track: tracks.tracks[1], timestamp: 111_000, tracks: tracks)
    let localTestBookmarkThree = TrackPosition(track: tracks.tracks[2], timestamp: 1000, tracks: tracks)
    ...
    sut = AudiobookBookmarkBusinessLogic(book: fakeBook, registry: mockRegistry, annotationsManager: mockAnnotations)
    sut.syncBookmarks(localBookmarks: registryTestBookmarks.compactMap { $0.toAudioBookmark() }) { _ in
      let remoteBookmarks = self.mockAnnotations.bookmarks[self.fakeBook.identifier]?.compactMap(\.value) ?? []
