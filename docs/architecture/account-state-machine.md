...
| `/loans/` URL builder in `BookRegistrySync` | Silent (sync runs in background); registry stays in `.unloaded` until ready | **Bounded 30s** via `awaitReady(timeout:)` — see amendment below (HelpSpot #18414) |
...
| FCM push-token registration (`NotificationService.updateToken`) | Silent — runs from a Firebase callback at launch and on library switch; nothing is user-visible and nothing is watching | **Bounded 45s** via `awaitReady(timeout:)` — see amendment below (PP-4958) |

**Single timeout policy:** prefer a caller's own pipeline timeout over stacking a second `withTimeout` on `awaitReady()`. The audiobook open path has a 20s session-manager timeout and inherits it (do not add a second gate). This is a *default*, not an absolute: a call site with **no** enclosing timeout of its own MUST bound its own await (below).

**Amendment (3.2.3, HelpSpot #18414 — `BookRegistrySync` gets a bounded `awaitReady(timeout:)`).** The `/loans/` URL builder row above originally read "No additional timeout — `BookRegistrySync` already has its own retry policy." Field forensics (#18414) proved that rationale wrong for this one call site: `BookRegistrySync`'s retry policy only re-*triggers* a sync; it does not interrupt an in-flight `awaitReady()`. When the account's `authentication_document` fetch dropped its completion (account stuck at `.detailsLoading`), the **unbounded** `awaitReady()` inside the sync Task hung forever — registry sync never completed, My Books showed empty, and an incidental empty save could then persist over the good on-disk shelf (permanent data loss + a wedged borrow). `BookRegistrySync` therefore now calls `awaitReady(timeout: authReadinessTimeout /* 30s */)` and, on `AccountLoadError.readinessTimedOut`, reverts to `.loaded` so its retry policy drives the next attempt without ever reaching the reconciliation/save block with an empty shelf. This is a deliberate, forensics-driven exception to the "no additional timeout" default — the engineering call is that a background sync with no other timeout must bound its own await rather than risk an unbounded wedge. The code (`BookRegistrySync.authReadinessTimeout`, the `awaitReady(timeout:)` call, and the `.readinessTimedOut` revert) is the source of truth; this table now matches it.

**Amendment (3.3.0, PP-4958 — FCM push-token registration gets a bounded `awaitReady(timeout:)`).** `NotificationService.updateToken()` ran with no readiness gate at all: it fired from the Firebase registration callback at launch, 200–400ms *before* the authentication document landed, so `Account.details` was nil and `Account.getProfileDocument` returned on its first guard with no network call and no log. Push registration was silently skipped and never retried, because the existing recovery (PP-4275) requires a transition *into* `.loggedIn` and refreshing state does not produce one. Crashlytics recorded ~53,000 occurrences across ~6,600 patrons in 30 days; the patron-visible effect is that "your hold is ready" never arrives and the hold expires unseen. This call site now awaits readiness before registering.

Its timeout is **45s — deliberately longer than every other row, and longer than `authDocInflightTimeout` (30s)**. Two reasons. First, this is a background path with no enclosing pipeline timeout and no UI, so the "prefer the caller's own timeout" default does not apply — it must bound its own await. Second, at exactly 30s the wait would expire in the same instant the wedge reclaim becomes available, so it could never benefit from the reclaim its rationale depends on. Nothing re-fires registration after a timeout — the auth-doc drive resolves the *account*, not the attempt — so this budget has to be the last word. Readiness failures are routed three ways rather than silenced together. A library switch (`.evicted`) is silent, because it is a normal patron action that the auth-document loader re-drives, and filing non-fatals on every switch would corrupt the metric used to verify the fix. A genuine load failure is reported. The timeout is reported under its own summary: before this amendment "not ready yet" was read at t=0 and was pure noise, but after a 45s bounded wait it means the authentication document is genuinely wedged, which is a small, real population that this call site does not help. Keeping it visible is what makes the amendment falsifiable — silencing it would drive the verifying metric to near-zero whether registration now succeeds or the account is never driven at all.

**Amendment (3.3.0, PP-5135 — the audiobook open path's Bucket-A readiness failure falls back to stored credentials).** Bucket A above says a critical-path reader "must succeed or surface error to user". For `AudiobookSessionManager.isUserAuthenticated()` that produced a patron-facing lie. `awaitReady()` resolves the AUTHENTICATION DOCUMENT, which answers whether a library requires auth and how; it is not the credential store. Offline, that fetch cannot complete, the account parks at `.detailsFailed`, and "surface the error" rendered as *"Please sign in to your library account to play this audiobook"* — shown to a patron who was signed in, holding a book already downloaded to the device. Measured on device in airplane mode (build 505): three `Authentication Document request failed to load Code=700`, then seven consecutive `Validation failed: notAuthenticated`. Not a race — every tap was refused until a relaunch on wifi re-resolved the account.

This call site now falls back to `hasCredentials()`, a keychain read needing no network. With no stored credentials it still returns false, so a genuinely signed-out patron is unaffected. The Bucket-A rule is not repealed: the failure IS still surfaced when it is real. What changed is the recognition that for a LOCAL open, auth-document readiness is not the question being asked — offline is precisely the case downloading exists for, so a network-dependent gate must not be what blocks local playback.

Following the PP-4958 amendment's principle that readiness failures are routed **by kind**, the credential lookup is scoped to the library uuid captured BEFORE the await, via `accountsManager.userAccount(for:)` rather than `currentUserAccount`. `.evicted` is thrown by exactly the event that also moves `currentAccountId` — a library switch — so a fallback reading the *live* current account would answer with the NEW library's credentials and open the old library's book against them. Scoping to the captured uuid keeps the question "is the patron signed in to the library whose book this is".
