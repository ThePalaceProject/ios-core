...
What that costs is countable, and the count is what the decision turns on. Against catalog `v1.2-2026-07-20` (24 entries), classifying each entry by the shipped text rather than by intent:

| | Entries | What it would take |
|---|---|---|
| **A. Prose that is wrong on Android** | **9** | Rewrite or per-platform text. Eight say "App Store"; one instructs the patron to *swipe up from the bottom of the screen*; one names CarPlay. |
| **B. Version-namespace collision** | 5 | Per-platform `fixed_in_version`, plus a per-platform `latest_known_app_version` on the envelope. Mechanical once the schema allows it. |
| **C. `ui_surface` names a surface** | 9 | Possibly nothing. The values are `my-books`, `catalog`, `settings-libraries`, `notifications-settings` — product surfaces, not iOS controls. Whether they exist under those names on Android is a question for the Android dev, not a cost this document can assert. |
| **Portable as written** | 5 | — |

Categories overlap; the union is 19 of 24, and 5 entries are clean.

**The number that constrains the branches is not 19, it is 4.** All four `generic_flow` entries — `GF-audiobook`, `GF-download`, `GF-reader`, `GF-other` — fall in category A. Those are the fallback lane, what the bot reaches for when nothing specific matches, and only `GF-library` survives. So "gate the Android launch to the portable subset" does not yield a smaller bot; it yields a bot that escalates almost everything, because the generic lane is where unmatched conversations land. That branch is materially more expensive than its one-line description suggests, and this is the fact a costing owed the decision.

Classification is by inspection of the shipped strings and is a snapshot, not a gate; it drifts as the corpus grows and should be recounted at Phase 2 rather than trusted from here.
