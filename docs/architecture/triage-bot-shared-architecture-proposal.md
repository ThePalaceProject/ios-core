...
<!-- audit-verified: PP-4882/4883/4884/4885/4886 summaries and story/sub-task structure verified via Jira API 2026-07-29; Android report-redaction prerequisite verified against android-core source; implementation specifics deliberately omitted and raised with the Android dev directly -->

## Decision cover memo

**Decision requested.** Approve three things: (1) the direction of this document, the hybrid architecture of [section 14](#14-rationale-motivations-and-alternatives); (2) Phase 0 and Phase 1 of the [delivery plan](#13-delivery-plan), whose first milestone, the iOS launch gate, precedes them; (3) authorization to start three organizational conversations now, because each has a lead time engineering does not control: HelpSpot access (API docs, sandbox, support contact, [section 5.5](#55-helpspot-integration-a-known-unknown)), the Palace AWS account operator ([section 17](#17-organizational-dependencies)), and the Android dev ([section 8](#8-android-implementation)). **Explicitly not being approved yet:** Android dates, the AI proxy (Phase 4), and Phase 3 scope. Those return as separate decisions when their prerequisites exist.

**Accountable owner.** The frontmatter's `owners: [support, infrastructure]` are team labels, not accountability. The accountable owner for this proposal and the iOS work is the iOS dev. The Android dev and the backend owner are unassigned and needed: no Android or CM work in this plan is real until a named person accepts it.

**Cost.** Phases 0 and 1 are near-zero infrastructure: a telemetry consumer and a CDN artifact (jsDelivr is free, S3 is cents, plus CI time). Phase 3 is not estimable until the HelpSpot prerequisites of [section 5.5](#55-helpspot-integration-a-known-unknown) exist; any Phase 3 number produced before then is unfounded. Phase 4 is uncosted: it carries ops burden and gets costed when scoped.

**Success metric and kill criteria, proposed.** Neither this document nor the as-built companion has defined a success metric or a stopping rule; the following proposal is for the team to accept or change. Success: within 90 days of the iOS flag going on, at least 15% of triage-bot sessions that would otherwise have produced a support ticket end without one (deflection rate, measured by the Phase 0 consumer via PP-4885). Kill: the program stops, rather than proceeding to Android or Phase 3, if after those 90 days deflection is under 5% and the person curating the corpus judges further corpus investment unlikely to change that. Both numbers are proposals, not findings.

**The do-nothing baseline.** What patron support costs today (ticket volume, staff time per ticket, resolution rates) is unknown. No deflection number can be judged against an unmeasured baseline, so establishing that baseline is a prerequisite for the success metric above, and it is currently nobody's task. The metric cannot be evaluated until the baseline has an owner.
...
**Audience:** Palace engineering and technical leadership; the backend engineer scoping the server work; the Android dev evaluating the Android work.
...
| The Android dev | [3](#3-design-constraints) (C9), [8](#8-android-implementation), [9](#9-cross-platform-conformance-fixtures), open decisions 8, 9, 15, 19 | 5.1 internals, 6, 10 |
| An iOS dev | [4.3](#43-client-fetch-cache-and-integrity), [10](#10-ios-component-disposition), [9](#9-cross-platform-conformance-fixtures) | 5.2, 5.3, 8.4 |
...
**One carve-out from "on-device," stated here because the framing above hides it.** The Anthropic key for the V2 AI fallback is never on a device, on either platform. It lives in a server-side proxy ([section 6](#6-server-ai-proxy-v2)); `AnthropicKeyStore` is deleted when that proxy lands; and until then the fallback is inert in every App Store build by construction, because release builds have no key path at all. A key shipped inside a binary is public the moment the binary is — extractable with `strings`, and `.apk` files are routinely unpacked — so no design here proposes one, and [section 14](#14-rationale-motivations-and-alternatives) rejects the no-server alternative partly on that ground. Read this before concluding the design puts a vendor credential on patron devices.

**Explicitly not built:** a corpus service, a standalone HelpSpot gateway, a server-side classifier (rejected: it forfeits offline behavior and moves patron text server-side on every turn, [section 14](#14-rationale-motivations-and-alternatives)), or a shared engine binary.
...
> **Constraint C1: redaction happens on-device, before transmission, always.** No design may move unredacted patron text off the device; this alone excludes a server-side classifier. Consequence: `ContextRedactor` (and its Kotlin port) runs before any byte reaches the CM, the proxy, or Anthropic.

> **Constraint C2: the patron bearer token can be validated by the CM and by nothing else, and for basic-auth libraries its plaintext contains the patron's plaintext password.**[^jwe] The token is the most sensitive datum in this system. Consequence: it may be sent to the CM, which minted it, and to nothing else; any service that needs "is this patron valid" must ask the CM ([section 6.1](#61-authentication)). It is never logged.
...
| C9 | `android-core` conventions: one contributor authored 66 of 66 commits in the last 90 days, documented OSGi-derived API/SPI/impl discipline, pure-JVM library modules, all tests in the monolithic `palace-tests` module, no Compose/`ViewModel`/`Flow`/reducer/Remote Config anywhere | The Android module is pure-JVM in the existing mold, consumes fixtures via `palace-tests`, and introduces no foreign patterns; design conversations with the Android dev precede code ([section 8](#8-android-implementation)) |
...
The channel is [open decision 16](#16-open-decisions). **Recommended: an npm package served via jsDelivr.** Both candidate channels are already in production in this org: the CM serves its entire admin UI from a pinned npm package version on jsDelivr, with a version-resolution API and env overrides[^admin-cdn]; MARC exports are written to the public S3 bucket and indexed by the CM[^marc-s3]. The deciding difference: npm gives every published version an immutable URL and a version-resolution mechanism ([section 4.3](#43-client-fetch-cache-and-integrity)). Public S3 has no version resolution at all, so choosing S3 forces one of two additions: a mutable "latest" pointer object (rejected here, because a mutable pointer on the same channel undoes both the immutability story and the integrity story of [section 4.3](#43-client-fetch-cache-and-integrity)) or the CM manifest endpoint promoted to mandatory from Phase 1. If S3 is chosen, the manifest endpoint is mandatory from Phase 1.

**jsDelivr becomes a patron-scale dependency.** The CM admin UI precedent is an operator-facing tool with a small audience; this artifact tells patrons what to do with their library accounts, at the install base's scale, over a free third-party CDN with no contract and no SLA. Availability: if jsDelivr is down or unreachable, sync fails silently and every client keeps serving its cached corpus (or the bundled snapshot on a fresh install), so an outage degrades the bot to a stale corpus rather than breaking it; what is lost is update propagation, not function ([section 4.3](#43-client-fetch-cache-and-integrity)). Supply chain: until the Phase 3 manifest anchor lands, the trust assumptions are two, the npm publish account (2FA and provenance attestation required from day one; who holds those credentials is [open decision 21](#16-open-decisions)) and jsDelivr's own integrity; that interim window is named in [section 4.3](#43-client-fetch-cache-and-integrity). The public-S3 alternative removes the third-party dependency by trading it for Palace-operated infrastructure, at the version-resolution cost stated above.
...
Curation is unchanged: a manual editorial process grounded in real HelpSpot tickets, with per-entry provenance in `internal_reference`. This design changes the distribution channel, not the editorial bar; no authoring tool or curator queue is assumed.
...
**Integrity versus tampering.** A client verifies the download's sha256 and, on mismatch, discards it and keeps its current corpus. The two checksum sources defend against different things:

- The **checksum file published beside the artifact** defends against corruption only. It travels the same channel as the artifact, so an attacker who can replace the artifact can replace the checksum.
...
It is also one artifact **across platforms**, and the schema has no dimension that says which platform an entry applies to. Every shipped entry is authored against iOS and three fields carry that assumption into the data: `user_facing_steps` are iOS-worded prose (KI-001 tells the patron to *swipe up from the bottom of the screen* to force-quit), `fixed_in_version` and the envelope's `latest_known_app_version` live in a single version namespace that the two apps do not share, and `ui_surface` names iOS surfaces under an iOS-only staleness log. A second client consuming this artifact unchanged gets wrong instructions and a version gate ([as-built, section 3.2](./triage-bot-v1-as-built.md)) that suppresses or surfaces entries against the other app's build numbers. This is [open decision 23](#16-open-decisions); it is the same shape as the locale question (decision 9) and wants the same kind of answer.

What that costs is countable, and the count is what the decision turns on. Counted against catalog `v1.2-2026-07-20` (24 entries), with every Android claim checked against `ThePalaceProject/android-core` at `a888a7d` (2026-08-21) rather than inferred:

| | Entries | What it would take |
|---|---|---|
| **A. Prose wrong on Android** | **9** | Rewrite or per-platform text. Eight say "App Store" (Play Store on Android); one instructs the patron to *swipe up from the bottom of the screen*; one names CarPlay (Android Auto). |
| **B. Version-namespace collision** | 5 | Per-platform `fixed_in_version` plus a per-platform `latest_known_app_version`. Mechanical once the schema allows it. |
| **C. `ui_surface` values** | 9 | **Nothing.** All four values name surfaces both apps have, under the same patron-facing labels — see below. |
| **Portable as written** | 5 | |

Categories overlap; the union is 19 of 24.

**Category C costs nothing, and that is a verified result rather than an assumption.** The four values are `catalog`, `my-books`, `settings-libraries`, and `notifications-settings`. Android has a dedicated tab for each of the first two (`CatalogFragmentMain`, `CatalogFragmentMyBooks`) and iOS has the same (`AppTab.catalog`, `AppTab.myBooks`); library management lives under `ui/accounts/` on Android and Settings on iOS. The labels match too: Android's `tabBooks` and iOS's `MyBooksView.navTitle` both read "My Books". Only `notifications-settings` differs in shape — on Android it is a row inside `SettingsMainFragment4` rather than a screen of its own, which is a wording problem for one entry, not a schema problem.

**The constraining number is 4.** Four of the five `generic_flow` entries — `GF-audiobook`, `GF-download`, `GF-reader`, `GF-other` — are in category A, all via "App Store". The generic lane is where unmatched conversations land, so gating the Android launch to the portable subset removes most of the bot's fallback rather than merely shrinking its corpus. That branch is materially dearer than its one-line description reads.

**One entry was wrong on iOS, and is fixed in this change.** `HT-2026-005-hold-pickup-window` carried `ui_surface: my-books` while telling the patron to open the title *from your holds* — but holds have their own tab on both platforms (`AppTab.holds`, labelled "Holds"; Android's `TAB_RESERVATIONS`, string resource `tabHolds`, also "Holds"), so the metadata anchored the answer to the wrong screen for iOS patrons, independently of any Android question. It now reads `ui_surface: holds`, and `holds` is registered in the how-to change log at 2026-07-10 — the date the Holds screen last visibly changed (`76a3586e0`, empty-state icons removed). The same commit changed My Books, whose log entry said 2026-05-01; that was understated and is corrected to the same date, which marks nothing stale because every my-books answer was reviewed 2026-07-20 or later.

Classification is by inspection of the shipped strings and is a snapshot, not a gate; it drifts as the corpus grows and should be recounted at Phase 2.
...
- `helpspot_tag` and `matchedEntryId` routing tickets instead of decorating attachments.
- Eventually, a substrate for the notify-me-on-fix flow, which is a stub today[^reducer].
...
`client.platform` reaches the CM and stops there. Nothing in this contract carries it into HelpSpot: the routing inputs named in [section 5.2](#52-per-library-routing) are the library and the entry's `helpspot_tag`, and no shipped tag encodes a platform — the tags are entry-derived (`known-issue-PP-4436`, `fixed-3.2.0`) or reducer-derived (`triage-bot-escalate-novel`, `guided-flow-<outcome>`). A triager working one queue fed by two clients therefore cannot filter by platform, which is the first cut anyone makes. Whether the server maps `platform` onto a HelpSpot field, a tag, or a mailbox is a routing decision that belongs with [open decision 18](#16-open-decisions), and it needs answering in the same conversation.
...
Responses. Every non-2xx body is a **problem-detail document in the CM's house format** (RFC 7807 shaped: `type`, `title`, `status`, `detail`), the same format both apps already parse on the sign-in paths; no new error format is introduced. Error cases are discriminated by the problem-detail `type` URI, not by status code:
...
| 429 | Problem detail: rate limited. Live from day one: the endpoint ships with a minimal fixed-window per-patron limit ([section 5.3](#53-credential-storage-rate-limiting-and-permanence)) | Back off, persist draft |
...
- **Abuse control:** net-new (constraint [C8](#3-design-constraints)), though less than the proxy needs; the endpoint ships with it: a minimal fixed-window per-patron limit (indicative: 5 submissions per patron per hour, enforced in the handler against Postgres, no new infrastructure), with 429 live from day one ([section 5.1](#51-endpoint-specification)). Support sign-off, a named precondition in the client code[^email-gateway], sets the numbers and the routing rules; it does not decide whether limits exist. An earlier revision marked 429 "reserved for future quota", which contradicted both that precondition and constraint C8; ship-with-limits is the resolution.
...
**The HelpSpot side of this endpoint is unspecified, deliberately, because nobody in these repos can specify it.** All 118 HelpSpot references in the org are iOS-side strings; no server-side integration has ever existed ([section 5](#5-server-support-ticket-endpoint)). Nothing in any Palace repo records the HelpSpot API endpoint, its auth scheme, its version, or the mapping from this contract's fields (`summary`, `description`, `matched_entry_id`, `helpspot_tag`, `context`, `client`) onto HelpSpot's fields, categories, and mailboxes. That mapping cannot be written from these repos, so it is documented as a prerequisite list instead:

1. **HelpSpot API documentation** for the version the org runs, including auth scheme and the create-ticket surface.
...
Nothing reads the bot's telemetry today, so a consumer comes before any data-driven prioritization. The destination is a fork, recorded as [open decision 4](#16-open-decisions):
...
The destination choice also decides whether the event payload needs a platform key, because the 12-key allowlist has none. On Option 1 the platform is carried by the Firebase app id, outside the payload, and two clients stay separable for free. On Option 2 both clients post to one endpoint and nothing in an event says which one sent it, so the allowlist gains a thirteenth key. That is not a sink detail: [section 8.2](#82-the-module) makes the allowlist a cross-platform contract with a language-neutral pinning fixture, so the key has to be agreed before either client freezes its fixtures rather than added afterwards.
...
The existing Android report flow attaches log archives that are not redacted: there is no redaction layer anywhere in that codebase, and the bot would use the same path[^android-logs]. Redaction at the logging boundary, or at zip-assembly time, is therefore a prerequisite for the Android client, and it is Android-side work that predates this proposal.

Specifics have been raised directly with the Android dev rather than enumerated here. What matters for planning is the shape: this is a separate workstream, plausibly larger than the bot module itself, and it gates the bot's use of the report path.

Porting the redactor is necessary but not sufficient: a redacted transcript attached beside an unredacted log zip still leaks the same credentials. The prerequisite applies to any Android ticket submission, bot or no bot.

This document does not size this workstream. It touches the logging posture of a codebase that today logs access tokens wholesale, and it needs its own scoping exercise, which sits with the Android dev: an inventory of what the log zip leaks, a decision between the logging boundary and zip-assembly time, and an estimate produced from that inventory. Until that scoping exists, no Android delivery date that includes ticket submission is credible, and the Phase 2 line for it in [section 13](#13-delivery-plan) is a placeholder for the scoping, not for the work.
...
| Fit with `android-core`'s architecture | Poor, by its own documented discipline | Good |

**Trace-first is the preferred integration shape, with one open gap: nobody has designed how a trace reaches the classifier.** The classifier's keywords are tuned to patron free text ("won't play", "just spins"); a `TaskStep` line reading "DRM license acquisition failed" shares no vocabulary with them, and every conformance case that exists, all 70 quality cases and all 6 hold-out cases, is a patron-text case. The recommended input path currently has no conformance coverage. Two ways to close the gap, to be chosen with the Android dev:
...
The alternative to two engine copies is running `TriageBotCore` itself on Android. The facts changed in March 2026: Swift 6.3 ships the first official Swift SDK for Android, owned by the Swift project's Android Workgroup, with generated interop tooling rather than hand-written JNI, over 2,200 packages building, and the recommended brownfield pattern being exactly this split, shared model layer with native UI per platform[^swift-android-sdk][^swift-android-wg][^swift-java]. The SDK is also, by its own characterization, work in progress (debugging, IDE integration, build times, binary size), and the bundled Swift runtime is around 60 MB on Android today[^skip-blog], though the runtime is not the strongest argument below. KMP was an earlier assumption never acted on; no Kotlin exists for this feature, so the question is open rather than settled by default.

The two things being duplicated have opposite risk profiles. The classifier is roughly 200 lines of deterministic matching, pinned by 70 quality cases plus the hold-out set; its failure mode is a wrong answer, visible in the conversation and recoverable by escalation. The redactor is 15 regexes that must agree across two regex dialects (ICU on iOS, `java.util.regex` on Android), covered today by a handful of synthetic fixtures; its failure mode is a patron credential leaking silently into a support ticket, and that failure class has already occurred once on a single platform, the prose-password incident ([as-built companion, section 8](./triage-bot-v1-as-built.md#8-privacy-and-redaction)). Engine sharing is two questions with different stakes, and the redactor is the one that needs the stronger defenses.

The governing principle: **share the data, not the code**. Everything that must not drift is becoming data already: the corpus, the redaction pattern set as versioned pattern data compiled by each platform ([section 8.2](#82-the-module), the adopted resolution of the dialect-drift objection), and the conformance fixtures that act as the oracle for both engines ([section 9](#9-cross-platform-conformance-fixtures)). What gets reimplemented per platform is only the matching logic that consumes that data, which is the part where fixtures can detect drift. The artifacts whose failure is silent are single-sourced; the code whose failure is visible is duplicated under an oracle.
...
| Cost to `android-core` | None; matches constraint [C9](#3-design-constraints) | A second toolchain, native artifacts, and a JNI bridge in a single-contributor repo with deliberately unified conventions | A new build plugin and a foreign module type |
...
### 8.7 Decisions owned by the Android dev

- Module naming, placement, and build wiring ([section 8.2](#82-the-module)); this document proposes the shape, the Android dev owns the fit.
...
**The first milestone is turning the shipped iOS bot on for patrons.** Every downstream decision in this plan steers by Phase 0 data, and Phase 0 data does not exist until the flag flips; that flip is the first step of the plan. It is tracked as story PP-4882, "Triage bot: close the gaps that block turning it on for patrons," with four sub-tasks. Three gate the flag going on: PP-4883 (honor the patron's omission choices on every send path, closing the clipboard-path gap), PP-4884 (give patrons a diagnostics opt-out and clear the privacy review), and PP-4885 (be able to tell whether the bot is helping). PP-4886 (notify-me delivery) rides the story but does not gate the flag.

**Gate:** no Phase 2 or later commitment is made until Phase 0 data exists. [Section 15](#15-signals-that-adjust-this-design) discloses that the plan's premise is unmeasured; this line is the gate.
...
| **2: conformance home + Android conversation** | The fixture deliverable of [section 9](#9-cross-platform-conformance-fixtures) (fixture files, redaction pattern data, format spec); reducer clock seamed on iOS. Before any Kotlin: the conversation with the Android dev, the V1 shape and trace-mapping decision ([section 8.5](#85-v1-shape)), the kill-switch decision ([open decision 19](#16-open-decisions)), log-redaction scoping ([section 8.4](#84-prerequisite-workstream-report-redaction)), and the localization product decision | None. New dependency: people's calendars |
...
Phase 1 teaches the real update cadence, whether curation throughput (not distribution) is the bottleneck, and sync reliability in the field. Costs: the original planning numbers ($15-30/month serving, ~$0.02/AI call, 25-50% auto-resolution at ~200 tickets/month) remain unverified projections; no auto-resolution measurement exists yet. Phase 1 costs CDN hosting (free on jsDelivr, cents on S3) plus CI time; Phase 3 is mostly review and sign-off; Phase 4 carries ops burden and is costed when scoped.

## 14. Rationale: motivations and alternatives

Three motivations are commonly given for moving support tooling server-side. Assessed against this system:
...
| Old-device compute | Solved (for a problem not shown to exist) | Unsolved (same unshown problem) | Same as B |
...
| Corpus update frequency wanted in practice | Whether Phase 1's cadence justifies authoring tooling, or the editorial process is the steady state |
...
The table triages the register; the numbered entries carry the detail. Roughly a third of these decisions are not engineering's to make. Where the owner is unknown, the column says unassigned rather than guessing. "By when" names the phase or milestone the decision blocks, where one makes it obvious.
...
| 7 | Corpus governance at scale | engineering (corpus curation) | Phase 1 exit |
| 8 | Android V1 shape and classifier input | the Android dev | Phase 2 |
...
| 12 | `Patron.uuid` exposure | the CM team | Phase 4 |
...
| 15 | Engine sharing mechanism | engineering, with the Android dev | Phase 2 |
...
| 19 | Android kill switch | the Android dev | Phase 2, before code |
...
| 23 | Corpus platform dimension | product, with engineering | Phase 2, before the Kotlin port |
| 24 | Known-issue entries have no lower version bound | engineering | with decision 23; they share the version namespace |
| 25 | Account cluster id truncated to 32 bits | engineering | before the Phase 2 fixture extraction pins a hash vector |
...
7. **Corpus governance ownership at scale.** `uiSurfaceChangeLog` is a hand-maintained map inside a test file; no authoring tool exists. *Rec: manual editorial process via PR + CI publish gate ([section 4.2](#42-publish-pipeline-and-quality-gates)) until update volume proves it insufficient.*
8. **Android V1 shape, including the classifier input path.** Full port vs trace-first, and within trace-first, whether the classifier consumes a designed trace-to-vocabulary mapping with trace-input fixtures, or only the patron's typed text with the trace attached unclassified ([section 8.5](#85-v1-shape)). *Rec: trace-first as the integration shape; no recommendation between the two input paths until the mapping is designed, because the mapped path currently has no conformance coverage. Settled by: the Phase 2 conversation with the Android dev, plus fixtures if the mapping is chosen.*
...
23. **Corpus platform dimension.** The corpus is a single global artifact with no per-entry statement of which platform it applies to, and every shipped entry is authored against iOS: iOS-worded remediation steps, a `fixed_in_version`/`latest_known_app_version` namespace the two apps do not share, and `ui_surface` values naming iOS surfaces ([section 4.4](#44-manifest-endpoint)). The register previously listed the locale dimension and not this one, which by the register's own rule read as settled; it was not. Three branches: dimension the schema (a per-entry applicability field plus per-platform version namespaces, which multiplies the curation surface the same way per-library corpora would), gate the Android launch to the subset of entries marked portable, or fork the artifact per platform and accept two curation queues. The per-entry cost of each is counted in [section 4.4](#44-manifest-endpoint), with every Android claim verified against `android-core@a888a7d` — 9 entries carry prose wrong on Android, 5 collide on version namespace, the 9 `ui_surface` values cost nothing because both apps have those surfaces under the same labels, and **four of the five generic-flow entries need rewriting**, which is what makes the gate-to-portable branch dearer than it reads. *Rec: none offered; the branches are costed above, and the curation commitment is product's to make. Settled by: the Phase 2 conversation with the Android dev and the corpus curator, before any Kotlin consumes the artifact.*
24. **Known-issue entries have no lower version bound.** The schema carries `fixed_in_version` and nothing that says which build first exhibited the issue, and `FixVersionGate` is correspondingly one-sided: it suppresses an entry when the patron's version is at or past the fix and applies no floor at all ([as-built companion, section 3.2](./triage-bot-v1-as-built.md#32-candidate-filtering)). A patron on a build older than the regression is therefore offered a workaround for a bug their build does not have — wrong advice, delivered confidently, on the platform that authored the entry. Adding an `introduced_in_version` field and the matching floor is small; what is not small is that it multiplies with [decision 23](#16-open-decisions), because a per-platform version namespace makes the floor per-platform too. Raised in review of the platform dimension by the Android reviewer; it had not been recorded anywhere, in either document or this register. *Rec: add the field and the floor, failing open on parse failure exactly as the ceiling does, and backfill it for the shipped known-issue entries. Settled by: engineering, alongside whichever branch of decision 23 is chosen, since the two share the version namespace.*
25. **The account cluster id is truncated to 32 bits.** `anon-` tokens carry the most-significant 32 bits of a 64-bit FNV-1a digest ([as-built companion, section 8](./triage-bot-v1-as-built.md#8-privacy-and-redaction)). The truncation is deliberate and the value is documented as a non-reversible cluster id rather than a cryptographic primitive, but the collision behavior was never written down: by the birthday bound, roughly 77,000 patrons gives a 50% chance that two accounts share a token, where the untruncated digest reaches that point past five billion. Nothing today depends on uniqueness, so this is not a live defect. The hazard is that the same section requires byte-for-byte cross-platform equality precisely so support can correlate one account's reports across clients — which is the use that turns a display convenience into a de facto identifier other systems join on. Raised in review by the Android reviewer. *Rec: keep the full 16-hex digest and treat the short form as a display truncation only, deciding before the conformance fixtures pin a hash vector, since the pinned vector is what makes the width permanent. Settled by: engineering, before the Phase 2 fixture extraction.*
...
| **CM team + release train** | Phase 3 endpoint; optional manifest endpoint; `Patron.uuid` exposure; any CM-hosted proxy | Ships every 2 to 4 weeks (nine major versions in six months). DB compatibility is rigorously versioned (N-1 online-migration contract, CI-gated); the HTTP API is not versioned at all, constraint [C5](#3-design-constraints) |
| **Android dev** | The V1 shape decision and all Android work | Constraint [C9](#3-design-constraints); the conversation happens before code |
...
[^android-logs]: `android-core`: logging configuration in `MainLogging.kt` and the report-assembly path in `palace-reports/Reports.kt`. A repository-wide search finds no redaction layer. Specifics raised directly with the Android dev.
