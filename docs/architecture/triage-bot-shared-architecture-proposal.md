...
**One entry is already wrong, on iOS, today.** `HT-2026-005-hold-pickup-window` carries `ui_surface: my-books` and tells the patron to open the title *from your holds* — but holds have their own tab on both platforms (`AppTab.holds`, labelled "Holds"; Android's `TAB_RESERVATIONS`, string resource `tabHolds`, also "Holds"). The metadata points at the wrong surface for iOS patrons now, independently of any Android question. Worth fixing whatever is decided here.
