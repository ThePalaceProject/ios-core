...
9. **Removing a blocking overlay is a change to everything it hid** (PP-5205, 2026-09-22, `.forgeos/wall-failures/2026-09-22-cache-written-only-by-the-signal-it-outruns.md`). The full-screen "Downloading…" panel covered the player for several seconds after a chapter tap. Taking it away — correctly — exposed a chapter title that had been reading four seconds behind the timecodes printed on the same row, since before the panel existed. **Rule:** when a change makes part of the player visible that a blocking state previously covered, enumerate every value rendered in that area and name, per value, WHICH writer updates it and ON WHICH EVENT. A value whose only writer is an event stream the user's action suppresses is the defect; a seek pauses the player, and a pause is the absence of exactly the position stream most caches are written from.

10. **Two labels rendering one fact must share a writer, or the cached one must be written by the action too** (PP-5205). Chapter identity reaches the player row twice: the timecodes compute it live from the player's position (which prefers `queuedTrackPosition`, the optimistic seek target), while the title reads `AudiobookSessionManager.currentChapter`, a `@Published` cache written only from `handlePositionUpdate`. Both are individually correct; the bug is their RELATIVE latency. **Rule, and the diagnostic:** when a screen shows a stale value next to a fresh one, rank the readers of that fact by latency before reading any code — the split localises the producer in one step. `ChapterNavigationPolicy` (`Palace/Audiobooks/AudiobookPositionPolicy.swift`) now owns both halves: an explicit selection publishes immediately, and a content-keyed hold keeps an update for the track being left from flipping the label back during the fire-and-forget seek. Note that its `selectionNeedsImmediatePublish` deliberately DISAGREES with `ChapterChangeDetector` on same-track/different-title pairs — the reactive rule must stay quiet mid-track for anthologies, the explicit one must not.

11. **`LCPStreamingPlayer.playCallback` overrides the base seek path wholesale, so anything the base does there is silently absent for LCP** (PP-5205). `OpenAccessPlayer.playCallback` sets `queuedTrackPosition` (`:270`) precisely so `currentTrackPosition` can report where the patron is GOING; the LCP override replaced the method and never did, so for LCP every position-derived label fell through to `avQueuePlayer.currentItem` — the previous track. **Rule:** when auditing an LCP playback defect, diff the override against `OpenAccessPlayer`'s method before theorising. The same override is also where the loading state is decided, and gating it on `!isSeekWithinSameTrack` alone announced a download for a chapter already fully on disk; locality (`LCPTrack.hasLocalFiles()` minus `forceStreamingTrackKeys`), not track identity, is the question that path must ask.

12. **`.onChange` does not fire for the value a view is BORN with** (PP-5205, `.forgeos/wall-failures/2026-09-22-onchange-has-no-birth-fire.md`). Centralising the player's load-timeout arming onto `.onChange(of: loadingOverlayCurrentState)` removed the `.onAppear` that armed it, and a cold open is born in `.skeleton` with no transition into it — so the 30s timer never armed and `.loadError`, which carries the only Retry button, was unreachable for the whole session. **Rule:** when a refactor centralises a lifecycle side effect, name every hook the old code ran from and say what fires the new one at BIRTH. `.onChange`, `.onReceive` and `.task(id:)` all answer "no" for the initial value; `.onAppear` and `.task` answer "yes". A pure rule plus a lifecycle hook is TWO things, and moving where the rule is consulted re-verifies neither.

13. **An override that replaces a base method wholesale drops the state that method maintained** (PP-5205, `.forgeos/wall-failures/2026-09-22-override-drops-base-state.md`). This happened twice to ONE property in one changeset: `LCPStreamingPlayer.playCallback` first never set `queuedTrackPosition` (so every label showed the previous track), then set it without clearing on a same-track seek (so the reported AND SAVED position froze — lost progress, lost bookmarks). Mutation is structurally blind: a property that is never assigned has no line to flip. **Rule:** diff the override against the base method and account for every instance property the base assigns. `scripts/check-override-drops-base-state.py` now does exactly that and is wired into `verify-pr.sh` — but it catches "never assigned", NOT "set without a matching clear", so it is a floor and not the answer. Where the state has a lifecycle, funnel the clear through a single exit rather than patching the exits you can see.

14. **Name the value the CONSUMER branches on before writing a predicate about it** (PP-5205, `.forgeos/wall-failures/2026-09-22-adjacent-predicate.md`). One guard on the LCP local-seek path was keyed three times and was wrong three times: `hasLocalFiles()` where `buildPlayerItem` gates on `assetFileStatus()`; playback state where the question was queue state; queue CONTENTS where the question was whether NAVIGATION landed — and the rebuild inserts the target item first, so that was already true before the seek ran. Every one compiles, reads true about the system, and answers a question one step away from the one the caller acts on. **Rule:** before writing a predicate that gates a user-visible outcome, name the exact value the consumer branches on, confirm you are reading THAT value rather than a neighbour usually equal to it, and write down a case where the two disagree. If you cannot construct one, you have not found the consumer. **Prefer a shape that cannot be wrong over a predicate that has to be right** — `OpenAccessPlayer.waitForItemReady` (:816-846) answers "did this item become usable" via `item.observe(\.status)` and cancels in the observer, so it is pause-independent by construction rather than by a condition someone has to get right. **And stop at two:** a second consecutive wrong keying on the same guard is evidence about the method, not the guard. Revert and file (PP-5213) rather than iterate on a release candidate.

15. **`LCPStreamingPlayer.playCallback` has NO direct test coverage, and the suite's green does not say otherwise.** The only tests are in `LCPStreamingPlayerAsyncContractTests`, whose stub OVERRIDES `playCallback` — so the routing tests never execute it. In PP-5205 that method took four of six toolkit commits and produced three of the ticket's regressions; its entire verification was reading. **Rule:** a change to that method is reviewed by reading and by device, and must say so rather than citing a toolkit suite tally. Building a `Player` harness that can drive `playCallback` deterministically is the standing gap.
