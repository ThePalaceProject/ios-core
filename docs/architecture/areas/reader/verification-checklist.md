...
| `Palace/Book/Models/TPPBook.swift` | 652 (`@objc var isDRMProtected: Bool`) | Single source of truth for "is this book DRM-protected?" — walks the nested acquisition chain via `TPPOPDSAcquisitionPath.supportedAcquisitionPaths(...)` for Adobe Adept, Readium LCP, Audiobook LCP. | **LIVE.** Adding a new DRM scheme = one line here. |
