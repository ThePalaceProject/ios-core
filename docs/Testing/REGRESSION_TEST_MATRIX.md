...
| E3-FA-Position | Findaway position must survive a chapter ending with the screen locked | Play a Findaway title to ~1 min before a chapter boundary. **Lock the phone** and put it down. Let the boundary pass and keep playing ~2 min into the next chapter. **Do NOT unlock to peek.** Force-quit from the lock screen or leave it until the app is jettisoned, then relaunch and reopen the title. Assert: playback resumes roughly where it actually was, NOT at the start of an earlier chapter. | Any Findaway title with chapters under ~15 min (a short chapter reaches the boundary sooner) | Manual — device, screen locked, read the resumed position in-app after relaunch | **PP-4951.** The chapter-completion handler saved `chapter.position` — the START of the chapter that just finished — and the tie-break named the chapter BEFORE it, so the place recorded moved back ~2 chapters at every boundary. With the screen locked nothing corrected it: every position mechanism on this path was a suspendable run-loop timer. Present since at least 3.0.0, not a 3.3.0 regression. **The measurement destroys itself if you unlock** — returning to the foreground writes a correct position immediately, so a peeked test always passes. **Regression signal:** any change to `AudiobookManager.handlePlaybackCompleted`, `positionToSaveOnChapterCompletion`, or the `preferChapterEndingHere` tie-break at the three sites that still pin it — **and, on the app side, `AudiobookSessionManager.handleManagerState`'s `.playbackCompleted` arm or `AudiobookPlaybackLifecycleSignal`.** PP-4951 has two halves in two repositories and this row originally named only the toolkit one. **Second assert, same run:** while the boundary passes, the player must never show paused while audio continues. The app treated a chapter ending as a stop, so on Findaway it flipped itself to paused at every chapter; it is normally masked by the next chapter's playback-began arriving milliseconds later, and becomes visible only when the SDK's chapter-complete notification arrives late — which is why it needs a device and cannot be seen in CI. E3-FA-Concurrency asserts the same symptom under PP-4990; if it appears, check both. |
| E3-FA-LockScreen | Findaway lock-screen / Now Playing must stay live with the screen off | Start a Findaway title, lock the phone, leave it playing 10+ min without touching it. Check the lock screen periodically **without unlocking**: elapsed time advances, title/chapter correct, artwork present, controls responsive. Compare against an open-access audiobook doing the same, which is the known-good path. | Any Findaway title, plus one open-access title as the control | Manual — device, screen locked, observe the lock screen only | **PP-4954.** The stale-lock-screen fix moved the refresh onto a position signal chosen because it comes from the audio player's clock — true for open-access (`AVPlayer.addPeriodicTimeObserver`), false for Findaway, where the same publisher was `Timer.publish(on: .main)` that iOS suspends. So the fix moved one suspendable timer onto another on DRM titles only. Now fed by the SDK's own progress notification. **Run the open-access control in the same session** — if both go stale it is not this. **Regression signal:** any change to `FindawayPlayer`'s position source, `positionPublisher`, or `AudiobookManager.setupNowPlayingHeartbeat`. |
| E3-FA-Concurrency | Findaway playback must not crash after the main-actor migration | Exercise the Findaway player hard in one session: play, pause, resume, skip ±15s repeatedly within a chapter, scrub the slider, cross a chapter boundary by skipping, change playback speed while playing, set the sleep timer and let it fire, and let the book reach its end. Assert: no crash, and playback state never shows paused while audio continues. | Any Findaway title | Manual — device, watch the console for `Fatal error: Incorrect actor executor assumption` | **PP-4990.** The Findaway player's state machine was moved onto the main actor, where its `Player` conformance had already placed it — it had been split across a private serial queue with readers on main. The SDK calls stay off-main behind a seam. Getting isolation wrong here crashes at runtime rather than failing the build, and this is the one player CI cannot exercise. **Regression signal:** the three `MainActor.assumeIsolated` sites in `FindawayPlayer`, the `FindawayEngineCommands` seam, or anything reintroducing a background queue for the state machine. A trap names which site fired — capture the frame list. |
