...
| File | Change | Est LOC |
|------|--------|---------|
| `scripts/check-lcp-acquisition-recursive.py` (NEW) | Detector. Identifies any function in `Palace/MyBooks/` or `Palace/Reader2/` or `Palace/Audiobooks/` or `Palace/OPDS2/` whose body matches the unsafe pattern: references `defaultAcquisition` AND references an LCP MIME literal (`application/vnd.readium.lcp.license.v1.0+json` or any `lcp` MIME) AND does NOT reference `indirectAcquisitions` OR `hasLCPAcquisition`. Annotation escape: `// no-lcp-recursive: <reason>`. **Phase-1a-revised scan scope: `Reader3/` does NOT exist; `Audiobooks/` is where PP-4407 actually lives. Verified survivor: `Palace/Audiobooks/LCP/LCPAudiobooks.swift:200-201` still has the OLD non-recursive `defaultAcquisition.type ==` pattern alongside the canonical recursive fix at line 217.** | +130 |
