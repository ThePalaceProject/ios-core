...
class DPLAAudiobooks {
  enum DPLAError: Error {
    ...
    var localisedDescription: String {
      switch self {
      case let .requestError(url, error): "Error requesting key data from \(url): \(error.localizedDescription)"
      case let .drmKeyError(message): message
      }
    }

    var readableError: String {
      switch self {
      case .requestError: "Error receiving DRM key."
      case .drmKeyError: "Error decoding DRM key data."
      }
    }
  }
  ...
  static func drmKey(completion: @escaping (_ keyData: Data?, _ validThrough: Date?, _ error: Error?) -> Void) {
    let task = URLSession.shared.dataTask(with: DPLAAudiobooks.certificateUrl) { data, response, error in
      ...
      if let response = response as? HTTPURLResponse,
         let cacheControlHeader = response.allHeaderFields[cacheControlField] as? String
      {
        let cacheControlComponents = cacheControlHeader.components(separatedBy: "=")
      ...
      guard let jwkData = data else {
        completion(
          nil,
          nil,
          DPLAError.drmKeyError("Error decoding \(DPLAAudiobooks.certificateUrl): response data is empty")
        )
        return
      }
      guard let jwkResponse = try? JSONDecoder().decode(JWKResponse.self, from: jwkData),
        ...
        let dataString = String(data: jwkData, encoding: .utf8) ?? ""
        completion(
          nil,
          nil,
          DPLAError.drmKeyError("Error decoding \(DPLAAudiobooks.certificateUrl) response:\n\(dataString)")
        )
        return
      }
      // All is good
