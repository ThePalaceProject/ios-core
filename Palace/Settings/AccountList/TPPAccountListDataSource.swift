...
class TPPAccountListDataSource: NSObject {
  ...
  func loadData(_ filterString: String? = nil) {
    ...
    if let filter = filterString, !filter.isEmpty {
      nationalAccounts = accounts.filter { AccountsManager.TPPNationalAccountUUIDs.contains($0.uuid) && $0.name.range(
        of: filter,
        options: .caseInsensitive
      ) != nil }
      accounts = accounts.filter { !AccountsManager.TPPNationalAccountUUIDs.contains($0.uuid) && $0.name.range(
        of: filter,
        options: .caseInsensitive
      ) != nil }
    } else {
      nationalAccounts = accounts.filter { AccountsManager.TPPNationalAccountUUIDs.contains($0.uuid) }
      accounts = accounts.filter { !AccountsManager.TPPNationalAccountUUIDs.contains($0.uuid) }
    }
