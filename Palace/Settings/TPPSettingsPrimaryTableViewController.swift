//
//  TPPSettingsPrimaryTableViewController.swift
...
class TPPSettingsPrimaryTableViewController : UITableViewController {
  var items: [TPPSettingsPrimaryTableItem] {
    didSet {
      itemsMap.removeAll()
      for item in items {
  ...
  var developerVC: TPPDeveloperSettingsTableViewController
  ...
  var splitVCAncestor: TPPSettingsSplitViewController? {
    var ancestor = self.parent
    while ancestor != nil {
      guard let splitVC = ancestor as? TPPSettingsSplitViewController else {
  ...
  override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if isDeveloper(path: indexPath) {
      splitVC.showDetailViewController(TPPSettingsPrimaryTableItem.handleVCWrap(self.developerVC), sender: self)
    } else {
