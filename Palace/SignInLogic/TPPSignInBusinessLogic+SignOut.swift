...
extension TPPSignInBusinessLogic {
  ...
  func performLogOut() {
    ...
    guard var request = makeRequest(for: .signOut, context: "Sign Out") else {
      return
    }
  ...
  private func processLogOut(
    ...
    do {
      ...
      Log.error(
        #file,
        "Unable to parse user profile at sign out (HTTP \(statusCode): Adobe device deauthorization won't be possible."
      )
       ...
       let clientToken = drm.clientToken, drm.vendor != nil
    {
      ...
      Log.info(#file, "\nLicensory token updated to \(clientToken) for adobe user ID \(userAccount.userID ?? "N/A")")
    } else {
  ...
  private func processLogOutError(
    _ errorWithProblemDoc: TPPUserFriendlyError,
    response: URLResponse?,
    for request: URLRequest,
    barcode: String?
  ) {
