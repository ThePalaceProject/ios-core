...
struct TPPPDFDocumentView: UIViewRepresentable {
  ...
  func makeUIView(context: Context) -> some UIView {
    ...
    pdfViewGestureRecognizer.onTouchEnded { touches in
      if let touchPoint = touches.first?.location(in: pdfView) {
        let elementTapped = pdfView.areaOfInterest(for: touchPoint)
  ...
  func makeCoordinator() -> Coordinator {
    Coordinator(currentPage: $metadata.currentPage)
  }
