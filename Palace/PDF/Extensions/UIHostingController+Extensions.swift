...
extension UIHostingController {
  ...
  private func createAndRegisterSubclass(originalClass: AnyClass, subclassedName: String) {
    guard let subclass = objc_allocateClassPair(originalClass, subclassedName, 0) else { return }
  ...
  private func overrideSafeAreaInsets(for subclass: AnyClass) {
    ...
    guard let method = class_getInstanceMethod(UIView.self, #selector(getter: UIView.safeAreaInsets)) else { return }
    class_addMethod(subclass, #selector(getter: UIView.safeAreaInsets), imp_implementationWithBlock(safeAreaOverride), method_getTypeEncoding(method))
  }
  ...
  private func overrideKeyboardHandling(for subclass: AnyClass) {
    ...
    guard let method = class_getInstanceMethod(subclass, keyboardSelector) else { return }
    class_addMethod(subclass, keyboardSelector, imp_implementationWithBlock(keyboardOverride), method_getTypeEncoding(method))
  }
}
