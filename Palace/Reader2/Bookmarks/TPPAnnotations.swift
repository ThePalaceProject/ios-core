...
@objcMembers final class TPPAnnotationsWrapper: NSObject, AnnotationsManager {
  ...
  func getServerBookmarks(
    ...
    completion: @escaping ([Bookmark]?) -> Void
  ) {
...
@objcMembers final class TPPAnnotations: NSObject {
  ...
  class func postListeningPosition(
    forBook bookID: String,
    selectorValue: String,
    completion: ((_ response: AnnotationResponse?) -> Void)? = nil
  ) {
    postReadingPosition(
      forBook: bookID,
  ...
  class func postReadingPosition(
    forBook bookID: String,
    selectorValue: String,
    motivation: TPPBookmarkSpec.Motivation,
    completion: ((_ response: AnnotationResponse?) -> Void)? = nil
    ...
    postAnnotation(
      forBook: bookID,
      withAnnotationURL: annotationsURL,
      withParameters: parameters,
      queueOffline: true
    ) { success, id, timeStamp in
      guard success else {
  ...
  private class func timeStamp(fromNetworkData data: Data?) -> String? {
    ...
    if let body = json[TPPBookmarkSpec.Body.key] as? [String: Any],
       let timeStamp = body[TPPBookmarkSpec.Body.Time.key] as? String
    {
      return timeStamp
    } else {
      Log.error(#file, "No Annotation ID saved: Key/Value not found in JSON response.")
  ...
  class func getServerBookmarks(
    ...
    let dataTask = TPPNetworkExecutor.shared.GET(annotationURL, useTokenIfAvailable: true) { data, _, error in
      if let error = error as NSError? {
      ...
      guard let first = json["first"] as? [String: Any],
            let items = first["items"] as? [[String: Any]]
      else {
      ...
      let bookmarks = items.compactMap {
        TPPBookmarkFactory.make(
          fromServerAnnotation: $0,
  ...
  class func deleteBookmark(
    ...
    let task = TPPNetworkExecutor.shared.DELETE(request, useTokenIfAvailable: true) { _, response, error in
      ...
      if response?.statusCode == 200 {
        ...
        Log.error(
          #file,
          "DELETE bookmark Request Failed with Error Code: \(error.code). Description: \(error.localizedDescription)"
        )
        completionHandler(false)
      }
    }
  ...
  class func uploadLocalBookmarks(
    ...
    for localBookmark in bookmarks {
      guard localBookmark.annotationId == nil else {
        continue
      }
  ...
  static var annotationsURL: URL? {
    TPPConfiguration.mainFeedURL()?.appendingPathComponent("annotations/")
  }
