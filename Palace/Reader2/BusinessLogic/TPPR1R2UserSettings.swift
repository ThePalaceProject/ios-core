...
class TPPR1R2UserSettings: NSObject {
  @objc let r1UserSettings: TPPReaderSettings
  ...
  init(r2UserSettings: UserSettings?) {
    self.r1UserSettings = TPPReaderSettings.shared()
  ...
  func setColorScheme(_ colorScheme: TPPReaderSettingsColorScheme) {
  ...
  /// - Note: This does not persist the change. Call `save()` for that.
  func setFontFace(_ fontFace: TPPReaderSettingsFontFace) {
  ...
  func modifyFontSize(fromOldValue fontSize: TPPReaderSettingsFontSize,
                      effectuating change: NYPLReaderFontSizeChange) -> TPPReaderSettingsFontSize {
    //  R1
    var newSize = fontSize
    let r1Changed: Bool = {
      switch change {
      case .increase:
        return TPPReaderSettingsIncreasedFontSize(fontSize,
                                                   &newSize)
      case .decrease:
        return TPPReaderSettingsDecreasedFontSize(fontSize,
                                                   &newSize)
      }
    }()
    if r1Changed {
  ...
  private func modifyR2FontSize(fromR1 r1Value: TPPReaderSettingsFontSize) {
    ...
    let r1Range = Float(TPPReaderSettingsFontSize.largest.rawValue - TPPReaderSettingsFontSize.smallest.rawValue)
    let percValue = Float(r1Value.rawValue - TPPReaderSettingsFontSize.smallest.rawValue) / r1Range
