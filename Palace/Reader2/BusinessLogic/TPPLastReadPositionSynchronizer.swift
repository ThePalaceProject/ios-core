...
class TPPLastReadPositionSynchronizer {
  ...
  private func syncReadPosition(for book: TPPBook, drmDeviceID: String?, publication: Publication) async -> Locator? {
    ...
    guard let bookmark = await TPPAnnotations.syncReadingPosition(ofBook: book, toURL: TPPAnnotations.annotationsURL)
    else {
    ...
    let bookLocation = TPPBookLocation(
      locationString: serverLocationString,
    ...
    await withCheckedContinuation { continuation in
      DispatchQueue.main.async {
        let alert = UIAlertController(
          title: DisplayStrings.syncReadingPositionAlertTitle,
