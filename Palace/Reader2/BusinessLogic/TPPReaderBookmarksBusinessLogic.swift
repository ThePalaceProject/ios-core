...
class TPPReaderBookmarksBusinessLogic: NSObject {
  ...
  init(
    ...
    bookmarksFactory = TPPBookmarkFactory(
      book: book,
  ...
  func currentLocation(in navigator: Navigator) -> TPPBookmarkR3Location? {
    guard
      ...
      let index = publication.resourceIndex(forLocator: locator)
    else {
      return nil
    }
  ...
  private func postBookmark(_ bookmark: TPPReadiumBookmark) {
    guard
      ...
      accountDetails.syncPermissionGranted
    else {
      bookRegistry.add(bookmark, forIdentifier: book.identifier)
      return
    }
  ...
  func shouldSelectBookmark(at _: Int) -> Bool {
    true
  }
  ...
  func syncBookmarks(completion: @escaping (Bool, [TPPReadiumBookmark]) -> Void) {
    ...
    let localBookmarks = bookRegistry.readiumBookmarks(forIdentifier: book.identifier)
    TPPAnnotations
      .uploadLocalBookmarks(localBookmarks, forBook: book.identifier) { bookmarksUploaded, bookmarksFailedToUpload in
        ...
        TPPAnnotations
          .getServerBookmarks(
            ...
            Log.debug(
              #file,
              serverBookmarks.count == 0 ? "No server bookmarks" : "Server bookmarks count: \(serverBookmarks.count)"
            )

            self.updateLocalBookmarks(
  ...
  func updateLocalBookmarks(
    serverBookmarks: [TPPReadiumBookmark],
  ...
  private func handleBookmarksSyncFail(
    message: String,
