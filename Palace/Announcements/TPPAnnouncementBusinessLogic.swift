...
// MARK: - TPPAnnouncementBusinessLogic
...
class TPPAnnouncementBusinessLogic {
  ...
  private var presentedAnnouncements: Set<String> = .init()
  ...
  func presentAnnouncements(_ announcements: [Announcement]) {
    ...
    guard let alert = alert(announcements: presentableAnnouncements) else {
      return
    }
  ...
  private func restorePresentedAnnouncements() {
    guard let filePathURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
      .appendingPathComponent(announcementsFilename),
      ...
      let presented = unarchived as? Set<String>
    else {
      return
    }
  ...
  private func shouldPresentAnnouncement(id: String) -> Bool {
    !presentedAnnouncements.contains(id)
  }
  ...
  private func storePresentedAnnouncementsToFile() {
    guard let filePathURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
      ...
      TPPErrorLogger.logError(
        withCode: .directoryURLCreateFail,
        summary: "Unable to create directory URL for storing presented announcements"
      )
      return
    }
    ...
    do {
      let codedData = try NSKeyedArchiver.archivedData(
        withRootObject: presentedAnnouncements,
  ...
  private func alert(announcements: [Announcement]) -> UIAlertController? {
    ...
    if let last = announcements.last {
      currentAlert?.addAction(UIAlertAction(title: DisplayStrings.ok, style: .default) { [weak self] _ in
