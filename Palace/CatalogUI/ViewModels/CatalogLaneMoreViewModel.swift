...
private extension CatalogLaneMoreViewModel {
    ...
    currentLoadTask = Task { [weak self] in
      ...
      do {
        ...
        let mapped = await Task.detached(priority: .userInitiated) {
          await CatalogViewModel.mapFeed(feed)
        }.value
  ...
  func sortBooks(_ books: [TPPBook], by sort: CatalogSort) -> [TPPBook] {
    ...
    case .titleZA:
      books.sorted { $0.title.localizedCaseInsensitiveCompare($1.title) == .orderedDescending }
    case .authorAZ:
      books.sorted { ($0.authors ?? "").localizedCaseInsensitiveCompare($1.authors ?? "") == .orderedAscending }
    case .authorZA:
      books.sorted { ($0.authors ?? "").localizedCaseInsensitiveCompare($1.authors ?? "") == .orderedDescending }
    }
  }
}
