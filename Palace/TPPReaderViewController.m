...
#import "TPPReaderTOCViewController.h"
...
@interface TPPReaderViewController ()
  <TPPUserSettingsReaderDelegate, TPPReaderTOCViewControllerDelegate, TPPReaderRendererDelegate, UIPopoverPresentationControllerDelegate>
...
@property (nonatomic) TPPReaderReadiumView *rendererView;
...
@property (nonatomic) TPPReadiumBookmark *currentBookmark;
...
@implementation TPPReaderViewController
...
- (instancetype)initWithBookIdentifier:(NSString *const)bookIdentifier
{
  ...
  self.title = [[TPPBookRegistry sharedRegistry] bookForIdentifier:self.bookIdentifier].title;
  
  self.hidesBottomBarWhenPushed = YES;
  
  [[TPPBookRegistry sharedRegistry] delaySyncCommit];
  
  [[TPPBookRegistry sharedRegistry]
   setState:TPPBookStateUsed
...
- (void)dealloc
{
  ...
  [[TPPBookRegistry sharedRegistry] stopDelaySyncCommit];
}

#pragma mark NYPLReaderRendererDelegate

- (void)renderer:(__attribute__((unused)) id<TPPReaderRenderer>)renderer
didEncounterCorruptionForBook:(__attribute__((unused)) TPPBook *)book
{
...
- (void)rendererDidFinishLoading:(__attribute__((unused)) id<TPPReaderRenderer>)renderer
{
  // Do nothing.
}

- (void)rendererDidBeginLongLoad:(__unused id<TPPReaderRenderer>)render
{
  self.activityIndicatorView.hidden = NO;
  [self.activityIndicatorView startAnimating];
}

- (void)renderDidEndLongLoad:(__unused id<TPPReaderRenderer>)render
{
  [self.activityIndicatorView stopAnimating];
  self.activityIndicatorView.hidden = YES;
}

- (void)renderer:(__unused id<TPPReaderRenderer>)render didReceiveGesture:(NYPLReaderRendererGesture)gesture
{
  switch (gesture) {
...
- (void)viewDidLoad
{
  ...
  // Bookmark button
  TPPRoundedButton *const bookmarkButton = [[TPPRoundedButton alloc] initWithType:TPPRoundedButtonTypeNormal isFromDetailView:NO];
  ...
  self.rendererView = [[TPPReaderReadiumView alloc]
                       initWithFrame:self.view.bounds
                       book:[[TPPBookRegistry sharedRegistry]
...
- (void)prepareHeaderFooterViews {
  ...
  self.headerViewLabel.textColor = [TPPConfiguration mainColor];
  ...
  if (self.bookIdentifier) {
    NSString *title = [[TPPBookRegistry sharedRegistry] bookForIdentifier:self.bookIdentifier].title;
...
- (void) prepareBottomView {
  ...
  self.bottomViewImageView.backgroundColor = [TPPConfiguration backgroundColor];
  ...
  self.bottomViewProgressLabel.textColor = [TPPConfiguration mainColor];
...
- (void)
renderer:(__unused id<TPPReaderRenderer>)renderer
...
spineItemTitle:(NSString *const)title
{
  if (UIAccessibilityIsVoiceOverRunning()) {
    UIAccessibilityPostNotification(UIAccessibilityPageScrolledNotification,
                                    [NSString stringWithFormat:NSLocalizedString(@"Page %lul of %lu", nil),
...
- (void)updateBookmarkIcon:(BOOL)on
{
  dispatch_async(dispatch_get_main_queue(), ^{
    TPPRoundedButton *bookmarkButton = self.bookmarkBarButtonItem.customView;
    if (on) {
...
- (void)updateCurrentBookmark:(TPPReadiumBookmark *)bookmark
{
...
- (void)TOCViewController:(__attribute__((unused)) TPPReaderTOCViewController *)controller
  didSelectOpaqueLocation:(TPPReaderRendererOpaqueLocation *const)opaqueLocation
{
...
- (void)TOCViewController:(__unused TPPReaderTOCViewController *)controller
...
- (void)TOCViewController:(__unused TPPReaderTOCViewController *)controller
didRequestSyncBookmarksWithCompletion:(void (^)(BOOL, NSArray<TPPReadiumBookmark *> *))completion
{
  [self.rendererView.syncManager syncBookmarksWithCompletion:completion];
}

#pragma mark TPPUserSettingsReaderDelegate
...
- (void)applyCurrentSettings
{
  ...
  switch([TPPReaderSettings sharedSettings].colorScheme) {
    case TPPReaderSettingsColorSchemeBlackOnSepia:
      ...
      self.navigationController.navigationBar.barStyle = UIBarStyleDefault;
      self.bottomViewImageView.backgroundColor = [TPPConfiguration readerBackgroundSepiaColor];
    ...
    case TPPReaderSettingsColorSchemeBlackOnWhite:
      self.activityIndicatorView.activityIndicatorViewStyle = UIActivityIndicatorViewStyleGray;
      self.navigationController.navigationBar.barStyle = UIBarStyleDefault;
      self.bottomViewImageView.backgroundColor = [TPPConfiguration readerBackgroundColor];
      self.bottomViewImageViewTopBorder.backgroundColor = [UIColor lightGrayColor];
      self.headerViewLabel.textColor = [UIColor darkGrayColor];
      self.footerViewLabel.textColor = [UIColor darkGrayColor];
      break;
    ...
    case TPPReaderSettingsColorSchemeWhiteOnBlack:
      ...
      self.bottomViewImageView.backgroundColor = [TPPConfiguration readerBackgroundDarkColor];
...
- (TPPR1R2UserSettings *)userSettings
{
  return [[TPPR1R2UserSettings alloc] init];
}
...
- (void)didSelectSettings
{
  ...
  TPPUserSettingsVC *vc = [[TPPUserSettingsVC alloc] initWithDelegate:self];
...
- (void)toggleBookmark
{
  TPPReaderReadiumView *rv = self.rendererView;
