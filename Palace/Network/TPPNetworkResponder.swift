...
class TPPNetworkResponder: NSObject {
  ...
  init(
    credentialsProvider: NYPLBasicAuthCredentialsProvider? = nil,
    useFallbackCaching: Bool = false
  ) {
    taskInfo = [Int: TPPNetworkTaskInfo]()
  ...
  // ----------------------------------------------------------------------------
  func addCompletion(
    _ completion: @escaping (NYPLResult<Data>) -> Void,
    taskID: TaskID
  ) {
    taskInfoQueue.async {
...
extension TPPNetworkResponder: URLSessionDataDelegate {
  ...
  private func handleHTTPResponse(
    ...
    currentTaskInfo: TPPNetworkTaskInfo,
    logMetadata: inout [String: Any]
  ) -> Bool {
  ...
  private func handleProblemDocument(
    for task: URLSessionTask,
    ...
    let errorWithProblemDoc = task.parseAndLogError(
      fromProblemDocumentData: responseData,
      networkError: networkError,
      logMetadata: logMetadata
    )
...
private extension URLSessionTask {
  // ----------------------------------------------------------------------------
  func parseAndLogError(
    fromProblemDocumentData responseData: Data,
    networkError: Error?,
    logMetadata: [String: Any]
  ) -> TPPUserFriendlyError {
  ...
  // ----------------------------------------------------------------------------
  func error(fromProblemDocument problemDoc: TPPProblemDocument) -> NSError {
...
// MARK: - TPPNetworkResponder + URLSessionTaskDelegate
