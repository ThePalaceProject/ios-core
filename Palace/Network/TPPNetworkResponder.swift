...
private func handleExpiredTokenIfNeeded(for response: HTTPURLResponse,
    ...
    if response.statusCode == 401 {
        ...
        Log.error(#file, "[SAML-REAUTH-FIX] 401 received, host=\(urlHost), path=\(urlPath), authStrategy=\(strategyDesc)")
        ...
        if authDef?.reauthStrategy == .browser {
            ...
            Log.error(#file, "[SAML-REAUTH-FIX] browser-strategy 401 — handing off to SAMLReauthCoordinator. host=\(urlHost), path=\(urlPath)")
