...
@objcMembers final class RemoteHTMLViewController: UIViewController, WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        ...
        let action1 = UIAlertAction.init(title: Strings.Generic.cancel, style: .destructive) { (_) in
            _ = self.navigationController?.popViewController(animated: true)
        }
        let action2 = UIAlertAction.init(title: Strings.Generic.reload, style: .destructive) { (_) in
