...
extension TPPNetworkExecutor {
    ...
    refreshQueue.async { [weak self] in
      ...
      guard let username = TPPUserAccount.sharedAccount().username,
        ...
        let error = NSError(
          domain: TPPErrorLogger.clientDomain,
          code: TPPErrorCode.invalidCredentials.rawValue,
          userInfo: [NSLocalizedDescriptionKey: "Unauthorized HTTP"]
        )
        completion?(NYPLResult.failure(error, nil))
        return
      }
      ...
      executeTokenRefresh(username: username, password: password, tokenURL: tokenURL) { result in
  ...
  private func retryFailedRequests() {
    ...
    while !retryQueue.isEmpty {
      ...
      executeRequest(request, enableTokenRefresh: true) { _ in
...
private extension URLRequest {
  ...
  var hasRetried: Bool {
    get {
      objc_getAssociatedObject(self, &AssociatedKeys.hasRetriedKey) as? Bool ?? false
    }
    set {
