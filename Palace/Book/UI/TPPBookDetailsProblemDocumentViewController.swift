@objcMembers class TPPBookDetailsProblemDocumentViewController: UIViewController {
  ...
  init(problemDocument: TPPProblemDocument, book: TPPBook?) {
    doc = problemDocument
    self.book = book
  ...
  override func viewDidLoad() {
       ...
       win.traitCollection.horizontalSizeClass != .compact
    {
      ...
      backButton.contentEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 2)
      ...
      closeButton.contentEdgeInsets = UIEdgeInsets(top: 0, left: 2, bottom: 0, right: 0)
      ...
      navBar!.widthAnchor
        .constraint(greaterThanOrEqualToConstant: backButton.intrinsicContentSize.width + closeButton
    ...
    label.attributedText = generateAttributedText(problemDocument: doc)
  ...
  func calculateContentSize() -> CGSize {
    ...
    height += max(backButton?.bounds.height ?? 0, closeButton?.bounds.height ?? 0)
    height += label?.bounds.height ?? 0
    height += submitButton?.bounds.height ?? 0
    height += elementSpacing * (backButton == nil ? 3 : 4)
    let width = max(submitButton?.bounds.width ?? 0, (backButton?.bounds.width ?? 0) + (closeButton?.bounds.width ?? 0))
  ...
  func submitButtonWasPressed() {
    let alert = UIAlertController(
      ...
      message: "Are you sure you want to email this error log to The Palace Project support?",
    ...
    alert.addAction(UIAlertAction(title: "Send email", style: .default, handler: { _ in
      ...
      ProblemReportEmail.sharedInstance.beginComposing(
        to: Strings.ProblemReportEmail.supportEmail,
