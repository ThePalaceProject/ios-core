...
final class BookDetailViewModel: ObservableObject {
  ...
  @objc init(book: TPPBook) {
    ...
    bookState = registry.state(for: book.identifier)
    bookIdentifier = book.identifier
    stableButtonState = computeButtonState(
      book: book,
      state: bookState,
    ...
    setupObservers()
    downloadProgress = downloadCenter.downloadProgress(for: book.identifier)
    #if LCP
  ...
  deinit {
    ...
    if let licenseUrl = Self.lcpLicenseURL(forBookIdentifier: bookIdentifier) {
      var lcpAudiobooks: LCPAudiobooks? = if let localURL = MyBooksDownloadCenter.shared.fileUrl(for: bookIdentifier),
                                             FileManager.default.fileExists(atPath: localURL.path)
      {
        LCPAudiobooks(for: localURL)
      } else {
        LCPAudiobooks(for: licenseUrl)
      }
  ...
  private func bindRegistryState() {
    registry
      ...
      .sink { [weak self] _ in
        ...
        if let override = localBookStateOverride, override == .returning, registryState != .unregistered {
          return
        }
        bookState = registryState
  ...
  private func computeButtonState(
    book: TPPBook,
    state: TPPBookState,
    isManagingHold: Bool,
    isProcessing: Bool
    ...
    if case .holding = state, isManagingHold {
      return .managingHold
    }
  ...
  private func setupStableButtonState() {
    Publishers.CombineLatest4($book, $bookState, $isManagingHold, $isProcessing)
      .map { [weak self] book, state, isManaging, isProcessing in
        self?
          .computeButtonState(book: book, state: state, isManagingHold: isManaging, isProcessing: isProcessing) ??
          .unsupported
      }
      .removeDuplicates()
  ...
  @objc func handleBookRegistryChange(_: Notification) {
  ...
  func selectRelatedBook(_ newBook: TPPBook) {
    guard newBook.identifier != book.identifier else {
      return
    }
    book = newBook
  ...
  @objc func handleDownloadStateDidChange(_: Notification) {
    DispatchQueue.main.async { [weak self] in
      guard let self else {
        return
      }
  ...
  func fetchRelatedBooks() {
    guard let url = book.relatedWorksURL else {
      return
    }
    ...
    TPPOPDSFeed.withURL(
      ...
      useTokenIfAvailable: TPPUserAccount.sharedAccount().hasAdobeToken()
    ) { [weak self] feed, _ in
      guard let self else {
        return
      }
  ...
  private func createRelatedBooksCells(groupedBooks: [String: [TPPBook]], moreURLs: [String: URL?]) {
    ...
    if let author = book.authors, !author.isEmpty {
      if let authorLane = lanesMap
        .first(where: { $0.value.books.contains(where: { $0.authors?.contains(author) ?? false }) })
      {
  ...
  private func removeProcessingButton(_ button: BookButtonType) {
    processingButtons.remove(button)
  }
  ...
  private func prefetchLCPStreamingIfPossible() {
    guard !didPrefetchLCPStreaming, LCPAudiobooks.canOpenBook(book),
          let licenseUrl = Self.lcpLicenseURL(forBookIdentifier: bookIdentifier)
    else {
      return
    }
...
extension BookDetailViewModel {
  func chooseLocalLocation(
    ...
    let remoteLocationIsNewer: Bool = if let localPosition = localPosition, let remotePosition = remotePosition {
      String.isDate(
        remotePosition.lastSavedTimeStamp,
        moreRecentThan: localPosition.lastSavedTimeStamp,
        with: serverUpdateDelay
      )
    } else {
      localPosition == nil && remotePosition != nil
    }
    ...
    if paths.count > 0 {
      let alert = TPPReturnPromptHelper.audiobookPrompt { [weak self] returnWasChosen in
        ...
        if returnWasChosen {
          ...
          didSelectReturn(for: book, completion: nil)
        }
...
private extension BookDetailViewModel {
  ...
  func getPublicationUrl(from _: LCPAudiobooks) -> URL? {
