...
enum RegistryFileRecovery {
  ...
  /// or not. The narrower `backupHasRecords` alone misses the wedge path where
  /// the shelf was never corrupted (so no `.bak` was minted) but registry sync
  /// wedged, leaving an empty in-memory shelf poised to clobber a healthy
  /// primary.
