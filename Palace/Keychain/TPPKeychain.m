...
- (void)setObject:(id const)value forKey:(NSString *const)key accessGroup:(NSString *const)groupID
{
  ...
  if([self objectForKey:key accessGroup:groupID]) {
    ...
    if (status != noErr) {
      TPPLOG_F(@"Failed to UPDATE secure values to keychain for group: %@. This is a known issue when running from the debugger. Error: %d", groupID, (int)status);
    }
  } else {
    ...
    if (status != noErr) {
      TPPLOG_F(@"Failed to ADD secure values to keychain for group: %@. This is a known issue when running from the debugger. Error: %d", groupID, (int)status);
    }
  }
}
...
- (void)removeObjectForKey:(NSString *const)key accessGroup:(NSString *const)groupID
{
  ...
  if (status != noErr && status != errSecItemNotFound) {
    TPPLOG_F(@"Failed to REMOVE object from keychain group: %@. error: %d", groupID, (int)status);
  }
}
