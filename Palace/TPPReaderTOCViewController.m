...
// Deprecated: this is used only by R1. R2 uses TPPReaderPositionsVC.
@interface TPPReaderTOCViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation TPPReaderTOCViewController
...
{
  __weak TPPReaderTOCViewController *const weakSelf = self;

  [self.delegate
   TOCViewController:self
   didRequestSyncBookmarksWithCompletion:^(BOOL __unused success, NSArray<TPPReadiumBookmark *> *bookmarks) {
         ...
         cellForRowAtIndexPath:(NSIndexPath *const)indexPath
{
  switch (segmentControlTypeWithInteger(self.segmentedControl.selectedSegmentIndex)) {
    case SegmentControlTypeTOC:{
      TPPReaderTOCCell *cell = [self.tableView
                                 dequeueReusableCellWithIdentifier:reuseIDTOC
                                 forIndexPath:indexPath];

      TPPReaderTOCElement *const tocElement = self.tableOfContents[indexPath.row];
    ...
    case SegmentControlTypeBookmark: {
      TPPReaderBookmarkCell *cell = [self.tableView
                                      dequeueReusableCellWithIdentifier:reuseIDBookmark
                                      forIndexPath:indexPath];
      TPPReadiumBookmark *const bookmark = self.bookmarks[indexPath.row];
      if (bookmark != nil) {
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
  if (editingStyle == UITableViewCellEditingStyleDelete) {
    if ((NSUInteger)indexPath.row < self.bookmarks.count) {
      TPPReadiumBookmark *bookmark = self.bookmarks[indexPath.row];
